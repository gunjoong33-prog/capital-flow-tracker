...
- 존댓말(합니다체)로, 과장 없이 담백하게 써라.
- 가독성을 위해 한 문장이 끝날 때마다(마침표 뒤) 반드시 줄바꿈해라 — 한 줄에 문장 하나씩만 써라.
- 아래 순서와 내용을 반드시 섹션별로 나눠서 포함해라(섹션 사이는 빈 줄로 구분, 섹션 안에서는 문장마다 줄바꿈).
...
좋은 환경인지 아닌지를 이 사이트의 최종 결론(매수/지켜보기/현금비중늘리기)에 근거해서 명확히 답해라.
단, "좋지 않은 환경입니다"처럼 상태 설명만으로 끝내지 말고, 그래서 지금 어떤 대응을 하는 게 좋을지
구체적인 전략까지 한 문장 더 덧붙여 마무리해라(예: 결론이 "현금비중늘리기"면 "방어적으로 현금 비중을
늘리고 신규 매수는 보류하는 전략이 필요합니다", "매수"면 "권장 매수 비중 내에서 분할 매수를 고려할 만합니다",
"지켜보기"면 "신규 매수보다는 관망하며 다음 지표 변화를 기다리는 게 낫습니다" 같은 식).
