...
// self-learning/page.tsx도 이 상수를 재사용해 "화면에 표시된 압축본"과 "실제로 프롬프트에
// 주입되는 압축본"이 신선도 기준에서 어긋나지 않게 한다(코드 리뷰 지적 — 표시만 하는 페이지가
// 이 컷오프 없이 최신 행을 그대로 보여주면, 크론이 멈춰 오래된 행이 남아있을 때 "주입됨"이라고
// 잘못 표시할 수 있었다).
export const MAX_SYNTHESIS_AGE_DAYS = 14;
